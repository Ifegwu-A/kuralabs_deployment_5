   ...
   stages {
    ...
    stage ('Push to DockerHub') {
      ...
      steps {
        sh '''#!/bin/bash
        echo $DOCKERHUB_CREDENTIALS_PSW | docker login -u $DOCKERHUB_CREDENTIALS_USR --password-stdin
        docker tag kura_deployment_5_main_nginx:latest finalboss360/kura_deployment_5_main_nginx:latest
        docker tag kura_deployment_5_main_gunicorn-flask:latest finalboss360/kura_deployment_5_main_gunicorn-flask:latest
        docker push finalboss360/kura_deployment_5_main_nginx:latest
        docker push finalboss360/kura_deployment_5_main_gunicorn-flask:latest
        docker logout
        '''
      }
    }
